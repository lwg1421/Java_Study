...
SELECT ROUND(AVG(sal),2)"월급평균", MAX(SAL)"최고", MIN(sal)"최저", count(empno)"인원수"
FROM emp
WHERE deptno = 30;

--2) 각 부서별 급여의 평균(소수 이하 반올림), 최고, 최저, 인원수를 구하여 출력하라.
SELECT deptno"부서번호",ROUND(AVG(sal))"급여평균", MAX(SAL)"최고", MIN(sal)"최저", count(empno)"인원수"
FROM emp
GROUP BY deptno
ORDER BY deptno;
...
--월급이 많은 순으로 출력하라.
SELECT deptno, ROUND(AVG(sal))"평균월급", SUM(sal)"전체월급", MAX(sal)"최고월급", MIN(sal)"최저월급"
FROM emp
GROUP BY deptno
ORDER BY 1;
